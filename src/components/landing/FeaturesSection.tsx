...
const FeatureCard = ({ icon: Icon, title, description, index }: FeatureCardProps) => (
  <motion.div
    ...
    className="relative"
  >
    <div className="flex items-start gap-3 sm:gap-6 p-4 sm:p-8 rounded-2xl sm:rounded-3xl bg-background border-2 border-transparent hover:border-primary transition-all duration-300">
      {/* Large Icon */}
      <div className="flex-shrink-0">
        <div className="w-12 h-12 sm:w-20 sm:h-20 rounded-xl sm:rounded-2xl bg-gradient-to-br from-accent to-accent/70 flex items-center justify-center shadow-lg">
          <Icon className="w-6 h-6 sm:w-10 sm:h-10 text-accent-foreground" />
        </div>
      </div>

      {/* Content */}
      <div className="flex-1">
        <h3 className="text-lg sm:text-2xl font-bold mb-2 sm:mb-3 text-primary">
          {title}
        </h3>
      ...
      {/* Number Badge */}
      <div className="absolute top-2 right-2 sm:top-4 sm:right-4 w-7 h-7 sm:w-10 sm:h-10 rounded-full bg-primary/10 flex items-center justify-center">
...
export const FeaturesSection = () => {
  ...
  return (
    <section className="contain-abs py-12 sm:py-24 px-4 sm:px-6 lg:px-8 bg-background">
